...
export function actUpdateProfile(description, simple_local_avatar) {
  return {
...
export function actUpdateProfileAsync({ description, simple_local_avatar }) {
  return async (dispatch) => {
    try {
      const response = await userService.changePassword({ description, simple_local_avatar });
      const data = response.data;
      console.log('data', data);
      dispatch(actChangePassword(data));
      return {
