...
export const ACT_FETCH_NEW_COMMENTS_CHILD = 'ACT_FETCH_NEW_COMMENTS_CHILD';

// action creator
export function actFetchCommentsParent({ comments, total, totalPages, currentPage, exclude }) {
  return {
...
export function actFetchNewCommentsChild(comments) {
  return {
    type: ACT_FETCH_NEW_COMMENTS_CHILD,
    payload: {
      comments
    },
  };
...
export function actFetchNewItemAsync({ author , content, post, parent = 0 }) {
  return async (dispatch) => {
    ...
    const comments = mappingCommentData(response.data);

    if(comments.parent){
      dispatch(actFetchNewCommentsChild( comments ));
    } else {
...
// action async
export function actFetchCommentsParentAsync({ page = 1, post = null, parent = 0, exclude = [] }) {
  return async (dispatch) => {
    const response = await commentService.getList({ page, per_page: 5, post, parent, exclude });
